...
# 1) Depoimentos: Neon é a fonte canônica. Mantemos o Apps Script apenas como
# leitura legada até o conteúdo histórico externo poder ser inventariado.
...
if fixed_start >= 0:
    ...
    if fixed_end < 0:
        raise SystemExit('Fim de DEPOIMENTOS_FIXOS não encontrado')
    s = s[:fixed_start] + s[fixed_end + 2:].lstrip('\n')

# As fotos dos três depoimentos fixos foram preservadas no Neon; retire somente
# o mutador antigo desses objetos em memória, mantendo a lógica de pausa/hover.
s = re.sub(
    r"\n\s*const avataresFixos = \{.*?\n\s*\};\n\n\s*try \{\n\s*DEPOIMENTOS_FIXOS\.forEach\(dep => \{.*?\n\s*\} catch \(_\) \{\}\n",
    '\n',
    s,
    count=1,
    flags=re.S,
...
if old_submit not in s:
    raise SystemExit('Bloco legado de submissão não encontrado')
s = s.replace(old_submit, new_submit, 1)

# Não exiba mais conteúdo fixo e dedupe a fonte legada contra o Neon.
...
if '.insert({...row,is_visible:true})' in s:
    raise SystemExit('Fallback direto de INSERT ainda aparece')
p.write_text(s, encoding='utf-8')

# 2) Batizado: remove metadados órfãos enquanto não há pacote comercial definido.
...
if re.search(r'^\s*batizado\s*:', s, re.M):
    raise SystemExit('Batizado ainda aparece sem pacote comercial')
p.write_text(s, encoding='utf-8')

# 3) Painel: extrai a moderação de depoimentos do admin.js.
...
# 4) Asset de CTA antigo: remove apenas se não houver referência de produção.

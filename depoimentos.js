...
const DEPOIMENTOS_FIXOS = [
  { nome: "Leane Santos", instagram: "leanesantos58", comentario: "Só temos a agradecer os excelentes profissionais que são, deixando a gente super à vontade e fazendo um belíssimo trabalho em dupla. 👏🏼👏🏼👏🏼", data: "" },
  { nome: "Gessilene", instagram: "gessy.ts", comentario: "Só tenho a agradecer ao meu Studio preferido, Araújo claro, por todo o carinho, dedicação, respeito, amor e cuidado com cada detalhe. Obrigada Michele e Wellington. Amei, adorei, chorei de tanta emoção. ♥️", data: "" },
  { nome: "Josineide", instagram: "josineides900", comentario: "Obrigado por vocês eternizarem esse momento tão lindo nas nossas vidas. 👏👏 Obrigado por todo o cuidado e carinho com a minha filha nesse dia tão esperado por todos nós.", data: "" }
];
...
function prepararFormularioRecolhivel() {
  ...
  style.textContent = `
    ...
    .dep-form-wrap.dep-collapsible .dep-form:not([hidden]){animation:depFormIn .38s cubic-bezier(.16,.84,.44,1) both}
    @keyframes depFormIn{from{opacity:0;transform:translateY(-8px)}to{opacity:1;transform:none}}
...
function criarCard(dep) {
  ...
  if (dep.instagram) {
    ...
    captionHTML = '<figcaption><a class="testi-name" href="https://www.instagram.com/' + escapeHTML(handle) + '/" target="_blank" rel="noopener">' + escapeHTML(dep.nome) + '</a><span class="testi-role">@' + escapeHTML(handle) + '</span></figcaption>';
  } else {
    captionHTML = '<figcaption><span class="testi-name">' + escapeHTML(dep.nome) + '</span>' + (dep.data ? '<span class="testi-role">' + escapeHTML(dep.data) + '</span>' : '') + '</figcaption>';
  }
  fig.innerHTML = estrelasSVG() + '<blockquote>' + escapeHTML(dep.comentario) + '</blockquote>' + captionHTML;
  return fig;
}
...
function montarCarrossel(depoimentos) {
  ...
  requestAnimationFrame(() => { posicionarTrack(); requestAnimationFrame(() => { carrosselTrack.style.transition = transicaoOriginal; }); });
...
function iniciarAutoplay() { if (window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches) return; autoplayTimer = setInterval(avancar, 6000); }
...
if (formDepoimento) {
  formDepoimento.addEventListener("submit", async (e) => {
    ...
    if (comentario.length > MAX_CHARS) return mostrarMsg("O comentário ultrapassou o limite de " + MAX_CHARS + " caracteres.", "erro");
    btnEnviar.disabled = true;
    btnEnviar.textContent = "Enviando…";
    try {
      const res = await fetch(DEPOIMENTOS_API_URL, { method: "POST", headers: { "Content-Type": "text/plain" }, body: JSON.stringify({ nome, instagram, comentario }) });
      ...
      } else mostrarMsg(data.erro || "Não foi possível enviar. Tente novamente.", "erro");
    } catch (err) {
...
async function carregarDepoimentos() {
  ...
  try {
    ...
    const data = await res.json();
    if (data.depoimentos && data.depoimentos.length) montarCarrossel([...DEPOIMENTOS_FIXOS, ...data.depoimentos]);
  } catch (err) {}
}

carregarDepoimentos();

// Fallback visual imediato: a camada conectada ao painel, carregada logo abaixo,
// substitui estes cards quando o Neon responde com trabalhos cadastrados.
(function montarTrabalhosRecentes() {
  ...
  const cards = destaques.map((d) => {
    const origem = mosaicos.find(item => item.querySelector(".mosaic-label")?.textContent.trim() === d.label);
  ...
  const style = document.createElement("style");
  style.textContent = `.recent-works-section{padding:96px 0;background:linear-gradient(180deg,#11100e 0%,#0e0d0b 100%);border-top:1px solid var(--hairline);border-bottom:1px solid var(--hairline)}.recent-works-head{text-align:center;max-width:820px;margin:0 auto 46px}.recent-works-eyebrow{display:flex;align-items:center;justify-content:center;gap:18px;color:var(--gold);font-size:.76rem;letter-spacing:.24em;text-transform:uppercase;margin-bottom:20px}.recent-works-eyebrow span{width:62px;height:1px;background:linear-gradient(90deg,transparent,var(--gold))}.recent-works-eyebrow span:last-child{background:linear-gradient(90deg,var(--gold),transparent)}.recent-works-head h2{font-size:clamp(2rem,4vw,3.2rem);line-height:1.12;margin-bottom:14px;color:var(--cream)}.recent-works-head h2 em{font-style:italic;color:var(--gold-light)}.recent-works-head p{color:var(--muted);max-width:680px;margin:0 auto}.recent-works-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:22px}.recent-work-card{background:linear-gradient(180deg,#181510,#12110e);border:1px solid rgba(201,162,75,.18);border-radius:16px;overflow:hidden;box-shadow:0 22px 50px rgba(0,0,0,.18);transition:transform .35s ease,border-color .35s ease,box-shadow .35s ease}.recent-work-card:hover{transform:translateY(-5px);border-color:rgba(201,162,75,.38);box-shadow:0 28px 60px rgba(0,0,0,.28)}.recent-work-photo{display:block;position:relative;width:100%;aspect-ratio:4/4.6;border:0;padding:0;background:#0e0d0b;cursor:pointer;overflow:hidden}.recent-work-photo::after{content:'';position:absolute;inset:0;background:linear-gradient(180deg,transparent 56%,rgba(14,13,11,.82) 100%);pointer-events:none}.recent-work-photo img{width:100%;height:100%;object-fit:cover;transition:transform .7s cubic-bezier(.16,.84,.44,1)}.recent-work-card:hover .recent-work-photo img{transform:scale(1.035)}.recent-work-tag{position:absolute;z-index:2;top:16px;left:16px;padding:7px 12px;border-radius:999px;background:rgba(14,13,11,.68);border:1px solid rgba(243,236,220,.22);backdrop-filter:blur(8px);color:var(--cream);font-size:.68rem;letter-spacing:.12em;text-transform:uppercase}.recent-work-body{padding:24px 24px 22px}.recent-work-body h3{font-size:1.65rem;color:var(--cream);margin-bottom:8px}.recent-work-body p{color:var(--muted);font-size:.9rem;line-height:1.6;min-height:58px}.recent-work-link{display:flex;align-items:center;justify-content:space-between;width:100%;margin-top:20px;padding:16px 0 0;border:0;border-top:1px solid rgba(201,162,75,.14);background:none;color:var(--gold-light);font:400 .78rem 'Jost',sans-serif;letter-spacing:.08em;text-transform:uppercase;cursor:pointer}.recent-work-link span{font-size:1.2rem;transition:transform .25s ease}.recent-work-link:hover span{transform:translateX(4px)}.recent-works-cta{display:flex;justify-content:center;margin-top:40px}@media(max-width:860px){.recent-works-section{padding:74px 0}.recent-works-grid{grid-template-columns:1fr;max-width:560px;margin:0 auto}.recent-work-photo{aspect-ratio:4/3.7}.recent-work-body p{min-height:0}.recent-works-eyebrow span{width:38px}}@media(max-width:520px){.recent-works-head{margin-bottom:34px}.recent-work-body{padding:20px}.recent-work-photo{aspect-ratio:4/4.2}}@media(prefers-reduced-motion:reduce){.recent-work-card,.recent-work-photo img,.recent-work-link span{transition:none!important}}`;
  document.head.appendChild(style);

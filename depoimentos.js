...
function lerTokensExclusao() {
  try { return JSON.parse(localStorage.getItem(DELETE_TOKENS_KEY) || "{}"); }
  catch (_) { return {}; }
}

function salvarTokenExclusao(id, token) {
  const tokens = lerTokensExclusao();
  tokens[id] = token;
  localStorage.setItem(DELETE_TOKENS_KEY, JSON.stringify(tokens));
}

function removerTokenExclusao(id) {
  const tokens = lerTokensExclusao();
  delete tokens[id];
  localStorage.setItem(DELETE_TOKENS_KEY, JSON.stringify(tokens));
}

function novoTokenExclusao() {
...
function prepararFormularioRecolhivel() {
  ...
  style.textContent = `
    ...
    .dep-invite h3{font-family:'Cormorant Garamond',serif;font-size:clamp(1.35rem,2.2vw,1.7rem);line-height:1.2;color:var(--cream);margin:0 0 5px;font-weight:500}
    .dep-invite p{margin:0;color:var(--muted);font-size:.86rem;line-height:1.5}
    .dep-toggle{flex:0 0 auto;display:inline-flex;align-items:center;gap:9px;border:1px solid rgba(201,162,75,.42);border-radius:999px;background:rgba(201,162,75,.06);color:var(--gold-light);padding:10px 16px;font-family:'Jost',sans-serif;font-size:.7rem;letter-spacing:.09em;text-transform:uppercase;cursor:pointer}
    .dep-toggle svg{width:14px;height:14px;transition:transform .32s ease}
    .dep-form-wrap.is-open .dep-toggle svg{transform:rotate(180deg)}
    .dep-form-wrap.is-open .dep-invite{padding-bottom:26px;margin-bottom:28px;border-bottom:1px solid rgba(201,162,75,.12)}
    .dep-form-wrap.dep-collapsible .dep-form[hidden]{display:none!important}
    .dep-photo-field{margin-top:18px}
    .dep-photo-row{display:flex;align-items:center;gap:14px}
    .dep-photo-preview{width:58px;height:58px;flex:0 0 58px;border-radius:50%;border:1px solid rgba(201,162,75,.34);background:rgba(201,162,75,.05);display:grid;place-items:center;overflow:hidden;color:var(--gold-light);font-family:'Cormorant Garamond',serif;font-size:1.1rem}
    .dep-photo-preview img{width:100%;height:100%;object-fit:cover}
    .dep-photo-actions{display:flex;flex-wrap:wrap;gap:8px}
    .dep-photo-btn{border:1px solid var(--hairline);background:transparent;color:var(--cream);padding:9px 12px;font:400 .7rem 'Jost',sans-serif;letter-spacing:.06em;text-transform:uppercase;cursor:pointer}
    .dep-photo-btn:hover{border-color:var(--gold)}
    .dep-photo-help{display:block;margin-top:6px;color:var(--muted);font-size:.7rem}
    .dep-photo-input{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}

    .testi-avatar{display:grid;place-items:center;overflow:hidden;border-radius:50%;border:1px solid rgba(201,162,75,.38);background:radial-gradient(circle at 35% 30%,rgba(230,200,120,.12),rgba(201,162,75,.035));color:var(--gold-light);font-family:'Cormorant Garamond',serif;font-weight:600;letter-spacing:.04em;box-shadow:0 10px 26px rgba(0,0,0,.18)}
    .testi-avatar img{width:100%;height:100%;object-fit:cover}
    .carousel-slide.testi-card>.testi-avatar{position:absolute;left:88px;top:40px;width:82px;height:82px;font-size:1.2rem;z-index:2}
    .carousel-slide.testi-card::before{display:none!important}
    .carousel-slide.testi-card figcaption{display:flex!important;flex-direction:column!important;align-items:flex-start!important;min-height:0!important;margin:0!important;padding:0!important;text-align:left!important;border:0!important}
    .carousel-slide.testi-card figcaption::before{display:none!important}
    .testi-delete-own{margin-top:9px;padding:0;border:0;background:none;color:rgba(243,236,220,.48);font:400 .66rem 'Jost',sans-serif;letter-spacing:.07em;text-transform:uppercase;cursor:pointer;transition:color .2s ease}
    .testi-delete-own:hover{color:#d79b8c}

    @media(max-width:860px){.carousel-slide.testi-card>.testi-avatar{left:54px;top:38px;width:70px;height:70px}}
    ...
    @media(max-width:560px){.carousel-slide.testi-card>.testi-avatar{left:50%;top:16px;transform:translateX(-50%);width:62px;height:62px}.carousel-slide.testi-card figcaption{align-items:center!important;text-align:center!important}}
  `;
  ...
  toggle.addEventListener("click", () => {
    ...
    if (abrir) requestAnimationFrame(() => document.getElementById("depNome")?.focus({preventScroll:true}));
  });
  ...
  if (instagramField && !document.getElementById("depFoto")) {
    ...
    campo.className = "dep-field dep-photo-field";
    ...
    const input = document.getElementById("depFoto");
    const preview = document.getElementById("depFotoPreview");
    const remover = document.getElementById("depFotoRemover");
    ...
    input.addEventListener("change", async () => {
      ...
      if (!f) return;
      if (!/^image\/(jpeg|png|webp)$/i.test(f.type) || f.size > 10 * 1024 * 1024) {
        mostrarMsg("Escolha uma imagem JPG, PNG ou WebP de até 10 MB.", "erro");
        input.value = "";
        return;
      }
      try {
        avatarData = await otimizarAvatar(f);
        preview.innerHTML = '<img src="' + avatarData + '" alt="Prévia da foto">';
        remover.hidden = false;
      } catch (_) {
        mostrarMsg("Não foi possível processar essa foto. Tente outra imagem.", "erro");
        input.value = "";
      }
    });
  }
...
async function otimizarAvatar(file) {
  ...
  canvas.width = size;
  canvas.height = size;
  canvas.getContext("2d", {alpha:false}).drawImage(bitmap, sx, sy, min, min, 0, 0, size, size);
  bitmap.close();
  let quality = .78;
  ...
  if (data.length > 200000) throw new Error("avatar grande");
  return data;
}
...
function avatarHTML(dep) {
  return dep.avatar_url
    ? '<span class="testi-avatar"><img src="' + escapeHTML(dep.avatar_url) + '" alt="Foto de ' + escapeHTML(dep.nome) + '"></span>'
    : '<span class="testi-avatar" aria-hidden="true">' + escapeHTML(iniciais(dep.nome)) + '</span>';
}
...
function criarCard(dep) {
  ...
  const token = dep.id ? lerTokensExclusao()[dep.id] : "";
  const excluir = token ? '<button type="button" class="testi-delete-own">Excluir meu depoimento</button>' : '';

  fig.innerHTML = avatarHTML(dep) + estrelasSVG() + '<blockquote>' + escapeHTML(dep.comentario) + '</blockquote><figcaption>' + nome + (role ? '<span class="testi-role">' + role + '</span>' : '<span class="testi-role"></span>') + excluir + '</figcaption>';

  if (token) {
    fig.querySelector(".testi-delete-own")?.addEventListener("click", async () => {
      if (!confirm("Excluir seu depoimento? Esta ação não pode ser desfeita.")) return;
      const btn = fig.querySelector(".testi-delete-own");
      btn.disabled = true;
      btn.textContent = "Excluindo…";
      try {
        const neon = await neonPublicClient();
        const { data, error } = await neon.rpc("delete_own_testimonial", { p_id: dep.id, p_token: token });
        if (error || data !== true) throw error || new Error("Token inválido");
        removerTokenExclusao(dep.id);
        mostrarMsg("Seu depoimento foi excluído.", "sucesso");
        await carregarDepoimentos();
      } catch (err) {
        console.error(err);
        btn.disabled = false;
        btn.textContent = "Excluir meu depoimento";
        mostrarMsg("Não foi possível excluir este depoimento.", "erro");
      }
    });
  }
  return fig;
...
function montarCarrossel(depoimentos) {
  ...
  requestAnimationFrame(() => {
    posicionarTrack();
    requestAnimationFrame(() => { carrosselTrack.style.transition = t; });
  });
...
function irPara(idx) {
  ...
  const prev = (carrosselIndex - 1 + carrosselTotal) % carrosselTotal;
  const next = (carrosselIndex + 1) % carrosselTotal;
  slides.forEach((s, i) => {
    s.classList.remove("active", "peek-left", "peek-right");
    if (i === carrosselIndex) s.classList.add("active");
    else if (i === prev && carrosselTotal > 2) s.classList.add("peek-left");
    else if (i === next && carrosselTotal > 2) s.classList.add("peek-right");
  });
...
function posicionarTrack() {
  ...
  const ativo = carrosselTrack?.querySelectorAll(".carousel-slide")?.[carrosselIndex];
  if (!wrapper || !ativo) return;
  carrosselTrack.style.transform = "translateX(" + (wrapper.offsetWidth / 2 - (ativo.offsetLeft + ativo.offsetWidth / 2)) + "px)";
}
...
if (campoComentario && contadorEl) {
  campoComentario.addEventListener("input", () => {
    const r = MAX_CHARS - campoComentario.value.length;
    contadorEl.textContent = r;
    contadorEl.classList.toggle("low", r < 50);
    contadorEl.classList.toggle("over", r < 0);
  });
}
...
if (formDepoimento) {
  formDepoimento.addEventListener("submit", async e => {
    ...
    try {
      ...
      avatarData = "";
      const preview = document.getElementById("depFotoPreview");
      if (preview) preview.innerHTML = "<span>Foto</span>";
      const remover = document.getElementById("depFotoRemover");
